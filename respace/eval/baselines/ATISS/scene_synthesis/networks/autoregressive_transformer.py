...
class AutoregressiveTransformer(BaseAutoregressiveTransformer):
    ...
    def add_object(self, room_mask, class_label, boxes=None, device="cpu"):
        ...
        if boxes is None:
            boxes = {
                "class_labels": torch.zeros((1, 0, self.n_classes), device=device),
                "translations": torch.zeros((1, 0, 3), device=device),
                "sizes": torch.zeros((1, 0, 3), device=device),
                "angles": torch.zeros((1, 0, 1), device=device)
            }
        else:

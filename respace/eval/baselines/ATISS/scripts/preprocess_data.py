...
def main(argv):
    ...
    # print(len(split_scene_ids))
    # unique_ids = set()
    # duplicate_ids = []
    # for scene_id in split_scene_ids:
    #     if scene_id in unique_ids:
    #         duplicate_ids.append(scene_id)
    #     unique_ids.add(scene_id)
    # print(len(split_scene_ids) - len(unique_ids), len(duplicate_ids))
    ...
    for (i, es), ss in tqdm(zip(enumerate(encoded_dataset), dataset)):
        ...
        if os.path.exists(room_directory):
            print("Room {} already exists".format(room_directory))
            continue

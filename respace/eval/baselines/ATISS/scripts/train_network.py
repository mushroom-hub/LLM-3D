...
def main(argv):
    ...
    for i in range(args.continue_from_epoch, epochs):
        ...
        if i % val_every == 0 and i > 0:
            ...
            network.eval()
            # TODO: veriy that it takes the full val dataset ?
            print("martin: validation on {} samples".format(len(val_loader)))
            ...
            # martin: additional code
            current_val_loss = StatsLogger.instance()._loss.value
            if current_val_loss < best_val_loss:
                best_val_loss = current_val_loss
                best_epoch = i
                print(f"\nNew best model found at epoch {i} with validation loss {best_val_loss:.4f}")
                
                # Save the best model with a special name
                torch.save(
                    network.state_dict(),
                    os.path.join(experiment_directory, "model_best.pth")
                )
                torch.save(
                    optimizer.state_dict(),
                    os.path.join(experiment_directory, "opt_best.pth")
                )
                
                # Log best model info to the stats file
                with open(os.path.join(experiment_directory, "best_model_info.txt"), "w") as f:

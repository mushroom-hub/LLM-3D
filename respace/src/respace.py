...
class ReSpace:
	def __init__(self, model_id="gradient-spaces/respace-sg-llm-1.5b", 
			  		env_file=".env", 
...
only output the JSON (with the removed objects) as a plain string and nothing else."""
	...
	def handle_prompt(self, prompt, current_scene=None, room_type=None, do_rendering_with_object_count=False, pth_viz_output=None):
		# ========== 新功能：专门处理commands，只添加物体 ==============
		if isinstance(prompt, dict) and "commands" in prompt:
			commands = prompt["commands"]
			# 如果提供了room_type，更新current_scene的房间类型
			if room_type is not None and current_scene is not None:
				current_scene["room_type"] = room_type
			
			print("=== 新功能：直接处理命令模式（只添加）===")
			print(f"收到命令数量: {len(commands)}")
			print(f"命令内容: {commands}")
			
			# 初始化场景（如果需要）
			if current_scene is None:
				current_scene = self._sample_random_bounds(self.dataset_train, room_type)
				print(f"初始化新场景，房间类型: {room_type}")
			
			# 只处理add命令，忽略remove命令
			add_commands = [cmd for cmd in commands if cmd.startswith("<add>")]
			# remove_commands = [cmd for cmd in commands if cmd.startswith("<remove>")]
			
			# if remove_commands:
			#     print(f"  忽略 {len(remove_commands)} 个remove命令（新功能只支持添加）")
			
			print("=============================================")
			print(f"处理 {len(add_commands)} 个添加命令")
			print("=============================================")
			...
			for command in add_commands:
				# 提取物体名称
				obj_prompt = re.search(r'<add>(.*?)</add>', command)
				if obj_prompt:
					obj_name = obj_prompt.group(1).lower()
					temp = 0.7
					# 调用add_object（现在返回2个值）
					...
					if is_success:
						success_count += 1
					print(f"添加物体 '{obj_name}': {'成功' if is_success else '失败'}")
				else:
					print(f"无效的add命令格式: {command}")
			
			# 返回结果
			if success_count > 0:
				print(f"SUCCESS! 成功添加 {success_count} 个物体，总计: {len(current_scene.get('objects', []))}")
				is_success = True
				return current_scene, is_success
			else:
				print("ERROR: 没有成功添加任何物体")
				is_success = False
				return current_scene, is_success

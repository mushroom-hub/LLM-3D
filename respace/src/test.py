	...
	for idx, response in enumerate(responses):
		...
		if scene_after:
			try:
				...
				if n_best_of_n_assets == 0:
					# 跳过资产检索，直接使用SG-LLM生成的场景
					result = {
						"scene": scene_after,
						"txt_pms_sampled_score": 1.0,  # 默认高分
						"delta_pbl_loss": 0.0,  # 默认无损失
						"status": "success_no_asset_retrieval",
						"prompt": prompt
					}
					print(f"  跳过资产检索，使用原始场景 (物体数: {len(scene_after.get('objects', []))})")
				else:
					# 正常进行资产检索测试

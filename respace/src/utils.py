...
def get_pth_mesh(asset_jid):
	...
	pth_base = os.getenv("PTH_3DFUTURE_ASSETS")
	full_path = os.path.join(pth_base, asset_jid, "raw_model.glb")

	if not os.path.exists(full_path):
		# 强制打印出导致失败的完整路径
		print(f"FATAL DIAGNOSIS: Asset file NOT FOUND at: {full_path}")
		return None  # 保持返回 None，以触发下游错误
